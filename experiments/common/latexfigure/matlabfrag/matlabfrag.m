...
p.addOptional('mathmode', true, @islogical );
...
isTextInMathMode = p.Results.mathmode;
...
try
  ...
  if (isTextInMathMode)
      writeOutNegXTick = @() fprintf(fid,['\n%%\n\\def\\%s{\\mathord{\\makebox[0pt][r]{' mathMode() '-' mathMode() ']}}}'],NEGXTICK_COMMAND);
  else
      writeOutNegXTick = @() fprintf(fid,['\n%%\n\\def\\%s{\\makebox[0pt][r]{-}}'],NEGXTICK_COMMAND);
  end  
  ...
  for ii=1:size(PsfragCmds,1)
    ...
    if NewFontStyle
      ...
      if CurrentWeight; Weight = latexBoldText();
      else Weight = ''; end;
      if CurrentlyFixedWidth; Fixed = '\ttfamily';
      else Fixed = ''; end;
      fprintf(fid,['\n%%\n\\providecommand\\%s%s{\\color[rgb]{%.3f,%.3f,'...
        '%.3f}\\fontsize{%.1f}{%.1f}%s%s%s\\selectfont\\strut}%%'],FontStylePrefix,...
      ...
      if ~isempty(ticklabels)
        ...
        if strcmpi(get(handle,[jj,'scale']),'log') && AutoTickLabel.(jj)
          ...
          if all(~isnan(str2double(ticklabelcell)))
            ...
            ticklabels = cellfun(@(x) [mathMode() '10^{',RemoveSpaces(x),'}' mathMode()],...
        ...
        elseif strcmpi(get(handle,[jj,'scale']),'linear') && AutoTickLabel.(jj)
          ...
          if abs(scale-1) > 1e-3
            ...
            LatexScale = [mathMode() '\times10^{',num2str(round(scale)),'}' mathMode()];
        ...
        if TicksAreNumbers
          if strcmpi(jj,'x')
            ...
            for kk=1:size(ticklabels)
              ...
              ticklabels{kk,:} = [mathMode(),RemoveSpaces(ticklabels{kk,:}),mathMode()];
            end
          end
        end
