...
if (option == 1)
  ...
  [distances_hamming, imp_distances_hamming] = distance_matrix( veins, veins_unreg );  
else
...
[c, n] = distribution_tester(distances_hamming, num_per_sample);
plot_pdf( c, n, 25, 200, ' ', 'Hamming Distance', 'Relative Frequency', {'Genuine scores','Imposter scores   '});
dp = dprime(c,n);
...
thresholds = [0 2000:10:25000]; limits = 0;
...
threshold = 17070; % The threshold resulted in the EER, as shown in Chapter 7 and Accuracy graphs

hFig = figure;

for test = [4:8:50] % Some nice, hard examples to test with
