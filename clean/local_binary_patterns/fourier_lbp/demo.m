...
if (option == 1)
  ...
  [distances_flbp, imp_distances_flbp] = distance_matrix( veins, veins_unreg );  
else
   load ../../experimental_data/score_matrices/fourier_lbp_scores 
end
...
[c, n] = distribution_tester(distances_flbp, num_per_sample);
...
thresholds = [0 500:10:3000]; limits = 0;
...
threshold = 1489; % The threshold resulted in the EER, as shown in Chapter 7 and Accuracy graphs

hFig = figure;

for test = [4:8:60] % Some nice, hard examples to test with
